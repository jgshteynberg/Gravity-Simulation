...
function resizeCanvas() {
  ...
  const width = Math.max(320, Math.round(rect.width || cssWidth || canvas.clientWidth || fallbackWidth));
  const height = Math.max(280, Math.round(rect.height || cssHeight || canvas.clientHeight || fallbackHeight));
...
function renderStatus() {
  ...
  simulationStatus.textContent = `Canvas ready: ${simulationWidth()} × ${simulationHeight()} with ${bodies.length} bodies`;
}
...
function draw() {
  ...
  ctx.strokeStyle = 'rgba(119, 225, 255, 0.16)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.arc(simulationWidth() / 2, simulationHeight() / 2, Math.min(simulationWidth(), simulationHeight()) * 0.28, 0, Math.PI * 2);
  ctx.stroke();
  ...
  bodies.forEach((body) => {
    ...
    glow.addColorStop(0.35, colorWithAlpha(body.color, 0.55));
    ...
    ctx.fillText(body.name, body.x + radius + 8, body.y - radius - 8);
  });
  renderStatus();
}
